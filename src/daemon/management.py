...
def inspect_daemon(paths: RuntimePaths | None = None) -> DaemonInspection:
    ...
    if metadata is None:
        return DaemonInspection(metadata=None, running=False, stale=False)

    running = is_process_running(metadata.pid)
    return DaemonInspection(metadata=metadata, running=running, stale=not running)
    ...
    while time.monotonic() < deadline:
        if process.poll() is not None:
            raise DaemonManagementError(
                f"Daemon exited before becoming ready (exit code {process.returncode})"
            )
        ...
        if (
            ...
            and inspection.metadata.pid == process.pid
        ):
            return inspection.metadata

        time.sleep(0.1)

    _terminate_process(process.pid, force=True)
    raise DaemonManagementError("Timed out waiting for daemon metadata")
