...
- **Indexing controls:** in progress. Worker-side indexing now has bounded task backpressure, per-file last-request-wins debounce in the watcher, capped torch thread configuration to reduce CPU saturation during embedding-heavy workloads, and daemon-backed smoke coverage now verifies multi-project indexing/query/filter behavior across multiple configured roots.
