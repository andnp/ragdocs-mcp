...
- **Soft projects:** in progress. Indexed content and query results now carry `project_id` metadata, and search now applies the bounded `1.2x` active-project uplift. Project-aware storage/path selection still exists in `src/config.py`, and explicit project-filter contracts plus broader docs rewrites remain outstanding.
