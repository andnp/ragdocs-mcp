...
- **Tranche 3 — Huey Ownership Transfer:** substantially complete. Production daemon wiring exists for document and git refresh tasks, the Huey consumer plus file/git watchers now run behind a supervised worker subprocess, and follow-up hardening fixed task persistence plus daemon freshness reloads/manifest synchronization. Richer task families/admin visibility remain in progress.
- **Tranche 4 — CLI-First Operations Surface:** partially complete. Daemon lifecycle commands, `queue status`, `index stats`, `/api/admin/overview`, and `/internal/shutdown` now exist and are dogfooded; queue-pressure metadata, watcher counters, and current index state now surface through the admin payloads, while deeper task inspection remains missing.
- **Tranche 5 — Soft Project Semantics:** substantially complete. `project_id` metadata now propagates from ingest into chunk/query results, active-project ranking uplift is implemented, authoritative index storage is globalized, explicit project-filter semantics exist on query surfaces, default discovery/indexing now spans configured project roots, and multi-root doc-id/reconciliation handling has been hardened across manager/reconciler/watcher paths. Remaining work is mostly operational polish and broader docs cleanup.
- **Tranche 6 — Cleanup & Hardening:** partially complete. Transitional fallback paths are gone, daemon attach/freshness/boot-lock issues have been hardened, focused daemon-backed smoke coverage exists, indexing now has watcher-side debounce plus queue backpressure/thread-limit controls, the first observability polish slice is in, multi-root identity handling is hardened, and daemon-backed multi-project smoke coverage now exercises indexing/query/filter flows across multiple registered roots. Multiprocess/snapshot cleanup and deeper task/throughput visibility are still incomplete.
