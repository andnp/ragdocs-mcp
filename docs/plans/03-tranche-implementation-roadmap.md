...
- **Tranche 6 — Cleanup & Hardening:** partially complete. Transitional fallback paths are gone, daemon attach/freshness/boot-lock issues have been hardened, focused daemon-backed smoke coverage exists, and indexing now has watcher-side debounce plus queue backpressure/thread-limit controls. Multiprocess/snapshot cleanup and richer observability are still incomplete.
