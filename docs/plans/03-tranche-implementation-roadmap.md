...
- **Tranche 6 — Cleanup & Hardening:** partially complete. Transitional fallback paths are gone, daemon attach/freshness/boot-lock issues have been hardened, focused daemon-backed smoke coverage exists, indexing now has watcher-side debounce plus queue backpressure/thread-limit controls, and the first observability polish slice is in. Multiprocess/snapshot cleanup, explicit multi-root identity hardening, and deeper task/throughput visibility are still incomplete.
