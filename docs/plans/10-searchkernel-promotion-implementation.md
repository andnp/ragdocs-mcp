...
- ~~**pgvector is built but DARK (major gap).**~~ **CLOSED (workflow 5, 2026-07-24, sonnet).** The live cutover is done: `PGVectorIndex` (a `VectorIndex`-shaped facade over `PGVectorStore` + direct `records`-table reads, since the live path needs by-id hydration / per-doc enumeration / chunk-path rename beyond the narrow `VectorStore` port) `b44f48b`(+integration tests `7d7158c`); `ApplicationContext._build_vector_store` now branches on `config.store.backend` — FAISS/`VectorIndex` stays the **unconditional default** (psycopg2 import lazy + `TYPE_CHECKING`-guarded so the FAISS path never touches it), `store.backend = "pgvector"` selects `PGVectorIndex` `72d7f7d`(+selection unit test `6c80271`, +live e2e recall test `b12f347`). Verified: 262 protected FAISS behavior tests green unchanged; the e2e test runs through the real composition root + `orchestrator.query()` against the container and returns correct top-1. `store.backend` is now genuinely consumed by live code. **Note:** `tests/integration/test_pgvector_store.py` (W1 store tests, untouched) has ~3 **pre-existing xdist-parallelism flakes** (shared-container table-name collisions) that pass 16/16 serially (`-n0`) — worth a per-worker-namespace fix eventually, not a cutover regression.
...
**Scope note (2026-07-29, decided by Andy):** Ollama dropped from scope. `sentence-transformers`
(via `HuggingFaceEmbeddingProvider`) already downloads and runs `Qwen3-Embedding-0.6B` locally
with no external server dependency, which is what Ollama would have provided — an Ollama adapter
would have been a redundant alternate transport (HTTP to a local Ollama daemon) for the same
local-embedding goal, not a prerequisite for it.

**Deliverables**
1. ~~`EmbeddingProvider` registry: adapters for Ollama (`qwen3-embedding`, `embeddinggemma`) + HF.~~
   **Done via HF only** (`HuggingFaceEmbeddingProvider`, no Ollama adapter — see scope note).
...
2. `LLMProvider` tiered facade: `complete(prompt, response_format, tier)` with fallback; adapters
   `gemma-api`, `claude-cli`, `codex-cli`, `copilot-cli` (SMART). Modeled on
   eng-analytics `AIProvider`; **distinct from** devkit `AgentBackend`. **Copilot is the only
   adapter implemented** (chosen as the first LLM per Andy); the FAST tier and the other CLI
   adapters are unimplemented and out of scope unless a future need calls for them.
3. Reranker adapter for `Qwen3-Reranker-0.6B` behind the `Reranker` seam (used by W4).

**Primary files:** `adapters/embedding/huggingface.py`, `adapters/llm/copilot_cli.py`,
`adapters/rerank/qwen3.py`; `indices/vector.py`, `config.py`.

**Acceptance:** switching embedding provider is config-only; Qwen3 embed + rerank both run locally
via `sentence-transformers`, no external inference server required.

**Tests:** provider selection tests; adapter unit + integration tests against the real model.
