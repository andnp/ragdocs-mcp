...
def test_daemon_status_reports_not_running(monkeypatch):
    runner = CliRunner()
    monkeypatch.setattr(
        "mcp_markdown_ragdocs.cli.inspect_daemon",
...
def test_daemon_status_json_includes_overview_when_available(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "mcp_markdown_ragdocs.daemon.status.request_daemon_socket",
...
def test_daemon_start_invokes_management_helper(monkeypatch):
    ...
    monkeypatch.setattr(
        "mcp_markdown_ragdocs.cli._ignore_daemon_startup_project_option",
        lambda project: observed.setdefault("ignored_project", project),
    )
    monkeypatch.setattr("mcp_markdown_ragdocs.cli.start_daemon", _fake_start_daemon)
...
def test_daemon_run_accepts_but_ignores_project_option(monkeypatch):
    ...
    monkeypatch.setattr("mcp_markdown_ragdocs.cli._run_daemon_forever", _fake_run_daemon_forever)
...
async def test_run_daemon_forever_releases_boot_lock_after_startup(
    ...
    monkeypatch.setattr("mcp_markdown_ragdocs.cli.acquire_boot_lock", lambda timeout_seconds=5.0: fake_lock)
    monkeypatch.setattr(
        RuntimePaths,
        "resolve",
    ...
    from mcp_markdown_ragdocs.cli import _run_daemon_forever
...
def test_daemon_stop_reports_stopped(monkeypatch):
    runner = CliRunner()
    monkeypatch.setattr(
        "mcp_markdown_ragdocs.cli.stop_daemon",
...
def test_daemon_restart_invokes_management_helper(monkeypatch):
    ...
    monkeypatch.setattr("mcp_markdown_ragdocs.cli.restart_daemon", _fake_restart_daemon)
...
def test_request_daemon_json_auto_start_ignores_project_context_for_startup(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mcp_markdown_ragdocs.cli.start_daemon", _fake_start_daemon)
    monkeypatch.setattr("mcp_markdown_ragdocs.cli.request_daemon_socket", _fake_request_daemon_socket)

    from mcp_markdown_ragdocs.cli import _request_daemon_json
...
def test_index_stats_reports_index_counts(monkeypatch, tmp_path):
    ...
    from mcp_markdown_ragdocs.cli import _build_index_stats_payload
...
def test_queue_status_requires_daemon_when_unavailable(monkeypatch):
    runner = CliRunner()
    monkeypatch.setattr("mcp_markdown_ragdocs.cli._request_daemon_json", lambda *args, **kwargs: None)
...
def test_request_daemon_json_uses_running_daemon(monkeypatch):
    ...
    monkeypatch.setattr(
        "mcp_markdown_ragdocs.cli.request_daemon_socket",
...
def test_query_passes_project_context_and_filter(monkeypatch):
    ...
    monkeypatch.setattr("mcp_markdown_ragdocs.cli._request_daemon_json", _fake_request)
