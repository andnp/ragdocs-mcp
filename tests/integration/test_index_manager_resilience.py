...
def test_remove_document_continues_on_partial_failure(tmp_path, config):
    ...
    db_file = keyword_path / "index.db"
    assert db_file.exists(), "Expected SQLite index.db after persist"
    # Corrupt by overwriting with non-SQLite bytes to trigger 'file is not a database'
    db_file.write_bytes(b"CORRUPTED" * 100)
    for wal in [keyword_path / "index.db-wal", keyword_path / "index.db-shm"]:
