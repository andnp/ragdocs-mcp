    ...
    async def ensure_ready(self, timeout: float = 60.0):
        ...
        if self.ensure_ready_delay > 0:
            ...
            if self.ensure_ready_delay > timeout * 10:
                ...
                raise TimeoutError(f"Mock timed out after {timeout}s")
...
class TestWaitReadyStateValidation:
    ...
    @pytest.mark.asyncio
    async def test_wait_ready_from_initializing_state_with_ctx(self):
        """
        Verify wait_ready() works from INITIALIZING state when ctx is set.
        """
        coordinator = LifecycleCoordinator()
        mock_ctx = MockApplicationContext()
